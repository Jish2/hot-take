...
async function getHotPosts(offset, limit, userId = "c74a8536-9ab4-44ed-9f96-ffe116e5aba8") {
	const std = 0.4;
	const mean = 0.31;
	...
	console.log(results.map((post) => post.rating));
	...
	switch (method) {
		case "GET":
			try {
				...
				if (limit < 0 || offset < 0) {
					res.status(400).json({
						message: "stop trying to fuck with us you cs pussy dick es8 wanna be nerd pussy",
					});
					return;
				}
