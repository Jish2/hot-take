...
function Dashboard({ setLoggedIn }) {
  ...
  function formatAMPM(date) {
    ...
    var strTime =
      months[month] + " " + day + " at " + hours + ":" + minutes + " " + ampm;
    return strTime;
  }
  ...
  async function fetchPosts(index) {
    try {
      const res = await fetch(
        `${API_URL}/posts?offset=0&sort=${sort_method[index ?? sortMethod].name}`,
      );
  ...
  async function loadMorePosts() {
    if (!searchRef.current.value) {
      try {
        const res = await fetch(
          `${API_URL}/posts?offset=${results.length}&sort=${sort_method[sortMethod].name}`,
        );
        const posts = await res.json();
  ...
  return (
    <>
      ...
      >
        ...
        <ModalContent style={{ margin: "0 16px 0 16px" }}>
          <ModalHeader fontSize="l" pb={0}>
            Post {selectedPost?._id} created on{" "}
            {formatAMPM(new Date(selectedPost?.date))}
          </ModalHeader>
          <ModalCloseButton />
      ...
      >
        <div
          style={{ width: "50px", minHeight: "60px", maxHeight: "60px" }}
        ></div>
        <Flex
          justify="center"
          align="center"
          ...
          style={{
            height: "fit-content",
            width: "100%",
            maxWidth: "var(--chakra-sizes-prose)",
          }}
        >
            ...
            {results.map((post, i) => {
              return (
                ...
                >
                  ...
                  >
                    {Math.round(
                      (post.reports.length / (post.interactions || 1)) * 100,
                    ) ?? 0}
                    %
                  </Text>
                  <div
    ...
    fetch(`${API_URL}/admin`, {
      ...
      .then((data) => {
        if (data?.success) {
          ...
          Cookies.set("adminPassword", passwordRef.current.value, {
            expires: expiration,
          });
        }
      })
